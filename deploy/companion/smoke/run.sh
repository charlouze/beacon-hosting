...
# node:22-alpine defaults to root, and BEACON_SAVE_OWNER=0:0 (smoke.env).
...
FORCE_COLOR=1 npx nx run cloud-init:render-compose | node ./render-smoke-compose.mjs > "$COMPOSE_FILE"
...
docker compose up -d enshrouded agent
...
original=$(docker compose exec -T enshrouded sh -c 'sha256sum /opt/enshrouded/server/savegame/3ad85aea /opt/enshrouded/server/savegame/3ad85aea-index' | tr -d '\r')
docker compose stop agent
docker compose run --rm -T --entrypoint sh --no-deps restore -c 'rm -f /opt/enshrouded/server/savegame/*'
docker compose up --force-recreate --exit-code-from restore restore
recovered=$(docker compose run --rm -T --entrypoint sh --no-deps restore -c 'sha256sum /opt/enshrouded/server/savegame/3ad85aea /opt/enshrouded/server/savegame/3ad85aea-index' | tr -d '\r')
...
test "$before" -ge 1
docker compose exec -T agent sh -c 'rm -f /opt/enshrouded/server/savegame/*'
sleep 90
...
docker compose exec -T agent sh -c '
  head -c 20000 /dev/urandom > /opt/enshrouded/server/savegame/3ad85aea
  head -c 512 /dev/urandom > /opt/enshrouded/server/savegame/3ad85aea-index
'
...
# exist too). This script has not called `docker compose stop enshrouded` yet, so
...
docker compose stop enshrouded
...
# called `docker compose stop enshrouded` itself, and `set -e` already means a
...
test "$(docker inspect -f '{{.State.Running}}' "$(docker compose ps -a -q enshrouded)")" = "false"
